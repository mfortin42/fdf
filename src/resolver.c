...
/*   Updated: 2016/02/12 15:58:23 by mfortin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_print_point(t_env *e, unsigned int y, unsigned int x)
{
	...
	e->y_prim = (-((double)e->v_tab[y][x] / 10) + (e->const_y / 2) * (double)(y) + (e->const_x / 2) * (double)(x)) * e->zoom + e->ori_y;
}
...
void	ft_print_next(t_env *e, unsigned int y, unsigned int x)
{
	...
	e->y_next_prim1 = (-((double)e->x_next / 10) + (e->const_y / 2) * (double)(y) + (e->const_x / 2) * (double)(x + 1)) * e->zoom + e->ori_y;

	e->x_next_prim2 = (-e->const_y * (double)(y + 1) - e->const_x * (double)(x)) * e->zoom + e->ori_x;
	e->y_next_prim2 = (-((double)e->y_next / 10) + (e->const_y / 2) * (double)(y + 1) + (e->const_x / 2) * (double)(x)) * e->zoom + e->ori_y;
...
/*
   ...
   {
   mlx_pixel_put(e->mlx, e->win, e->xn, e->yn, 0xFFFF);
   e2 = err;
   if (e2 > -(e->dx))
   {
   err -= (e->dy);
